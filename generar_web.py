...
Lee juegos.json como fuente maestra y genera una web estática indexable:
  ...
  - procedencia pública del ejemplar (tipo, nombre/alias autorizado y RRSS) en las fichas cuando está documentada
...
def public_provenance(game: dict[str, Any]) -> dict[str, Any]:
    """Normaliza la procedencia pública sin exponer datos no previstos por el schema."""
    raw = game.get("procedencia")
    if not isinstance(raw, dict):
        return {"tipo": "", "nombre_publico": "", "redes": {"instagram": "", "facebook": "", "x": ""}}
    redes = raw.get("redes") if isinstance(raw.get("redes"), dict) else {}
    return {
        "tipo": str(raw.get("tipo") or "").strip(),
        "nombre_publico": str(raw.get("nombre_publico") or "").strip(),
        "redes": {
            "instagram": str(redes.get("instagram") or "").strip(),
            "facebook": str(redes.get("facebook") or "").strip(),
            "x": str(redes.get("x") or "").strip(),
        },
    }


def normalize_social_profile(network: str, value: str) -> tuple[str, str]:
    """Devuelve (URL navegable, identificador visible) para una red social.

    Instagram y X aceptan URL completa, ``@usuario`` o ``usuario``. Facebook
    acepta URL completa y, por comodidad, también alias simples; para esta red
    sigue siendo recomendable guardar la URL completa porque puede representar
    perfiles, páginas u otras rutas. Nunca se devuelve una URL relativa.
    """
    raw = str(value or "").strip()
    if not raw:
        return "", ""

    # Permite dominios escritos sin esquema, evitando que el navegador los trate
    # como rutas relativas a la ficha del juego.
    domain_prefixes = {
    ...
    lowered = raw.lower()
    if any(lowered.startswith(prefix) for prefix in domain_prefixes.get(network, ())):
        raw = "https://" + raw.lstrip("/")
        lowered = raw.lower()

    if lowered.startswith(("https://", "http://")):
        # Las RRSS se enlazan siempre por HTTPS. Conservamos la ruta proporcionada.
        href = re.sub(r"^http://", "https://", raw, flags=re.I)
        clean = href.split("?", 1)[0].split("#", 1)[0].rstrip("/")
        ident = clean.rsplit("/", 1)[-1]
        if ident and ident.lower() not in {"instagram.com", "facebook.com", "x.com", "twitter.com"}:
            if network in {"instagram", "x"} and not ident.startswith("@"):
                ident = "@" + ident
            return href, ident
        return href, ""

    # Shorthand amigable para catalogación: @usuario o usuario.
    ident = raw.lstrip("@").strip().strip("/")
    if not ident:
        return "", ""
    if network == "instagram":
        return f"https://www.instagram.com/{quote(ident, safe='._')}/", "@" + ident
    if network == "x":
        return f"https://x.com/{quote(ident, safe='_')}", "@" + ident
    if network == "facebook":
        return f"https://www.facebook.com/{quote(ident, safe='._-')}/", ident
    return "", ident


def social_identity_label(network: str, value: str) -> str:
    """Etiqueta compacta para RRSS, normalizando antes URL o usuario."""
    _, ident = normalize_social_profile(network, value)
    return ident or {"instagram": "Instagram", "facebook": "Facebook", "x": "X"}.get(network, network)


def provenance_html(game: dict[str, Any]) -> str:
    """Contenido de la fila Procedencia usando únicamente datos declarados públicos."""
    p = public_provenance(game)
    tipo = p["tipo"]
    nombre = p["nombre_publico"]
    redes = p["redes"]
    if not tipo and not nombre and not any(redes.values()):
        return ""
    parts: list[str] = []
    if tipo:
        parts.append(f'<span>{h(tipo)}</span>')
    if nombre:
        parts.append(f'<strong>{h(nombre)}</strong>')
    social_links: list[str] = []
    labels = {"instagram": "Instagram", "facebook": "Facebook", "x": "X"}
    for network in ("instagram", "facebook", "x"):
        raw_value = redes.get(network, "")
        if not raw_value:
            continue
        url, ident = normalize_social_profile(network, raw_value)
        if not url:
            continue
        label = f"{labels[network]} {ident}" if ident else labels[network]
        social_links.append(f'<a class="tag" href="{h(url)}" target="_blank" rel="noopener noreferrer">{h(label)}</a>')
    identity = " · ".join(parts)
    social = f'<span class="tagrow provenance-social">{"".join(social_links)}</span>' if social_links else ""
    if identity and social:
        return f'<span class="provenance-main">{identity}</span>{social}'
    return identity or social
...
def generate_game_pages(games: list[dict[str, Any]], out: Path, project_root: Path, base_url: str, gallery_index: dict[str, list[str]] | None = None) -> None:
    ...
    for idx, game in enumerate(games, start=1):
        ...
        procedencia_publica = provenance_html(game)
        if procedencia_publica:
            extra_metadata_rows.append(f'<dt>Procedencia</dt><dd class="provenance">{procedencia_publica}</dd>')
        if anio:
